...
public class SVMLightWrapper
  ...
  public List batchClassifyInstances(List instances) throws ExecutionException {
    try {
      ...
      if (svmLightProcess.exitValue() != 0) {
       ...
       List Results = extractAllResultsFromResultsFile();
       return Results;
    }
    catch (IOException ex) {
      throw new ExecutionException(ex);
   ...
   */
  private void saveTestInstanceToDisk(List attributeValues) throws
   ...
   */
  private double extractResultFromResultsFile() throws IOException {
    java.io.FileReader reader
        = new java.io.FileReader(resultsFile.getCanonicalPath());
    MyStringReader inputFile=new MyStringReader(reader);
   ...
   * @throws gate.creole.ExecutionException
   */
  public void initialiseAndTrainClassifier() throws
   ...
   * options.)
   */
   String[] optionsString2OptionsList(String optionsString) {
     ...
     String[] optionsArray= new String[optionsArray1.length+3];
    ...
    while (trainingDataIterator.hasNext()) {
      documents[indexInArray] =
          new SVMLightDocument(datasetDefinition, nominalValue2IntegerHash,
                               (List) trainingDataIterator.next());
      ++indexInArray;
    }
    return documents;
  }
   ...
   * SVMLight.
   *
   * @throws gate.util.GateException If it is not possible to initialise the classifier
   * for any reason.
   */
  public void init() throws GateException {
    ...
    sListener = null;
    java.util.Map listeners = MainFrame.getListeners();
    if (listeners != null) {
    ...
    try {
      ...
      trainingDataFile = File.createTempFile("train", null);
      testDataFile = File.createTempFile("test", null);
      modelFile = File.createTempFile("model", null);
      resultsFile = File.createTempFile("results", null);
  ...
  private void createNominalValue2IntegerHash() {
    ...
    while (attributeIterator.hasNext()) {
      ...
      if (currentAttribute.semanticType() == Attribute.NOMINAL) {
        List features = currentAttribute.getValues();
   ...
   */
  public void saveDataset(java.io.FileWriter writer, List dataSet) {
    try {
  ...
  public void loadDataset(java.io.FileReader reader)
      throws gate.util.GateRuntimeException, IOException {
    MyStringReader inputFile=new MyStringReader(reader);
  ...
  private void addInstance(String classValue, List featureNumbers, List featureValues) {
    List attributeValues = new java.util.ArrayList();
   ...
   */
  private void copyModelToTempFile(String filename)
      throws IOException {
   ...
   */
  private void copyModelFromTempFile(String filename)
      throws IOException {
   ...
   */
  private void copyFile(File source, File destination) throws
      IOException {
   ...
   */
  public void load(java.io.InputStream is) throws IOException {
    if (sListener != null) {
    ...
    try {
      ...
      trainingData = (List) ois.readObject();
   ...
   */
  public void save(java.io.OutputStream os) throws IOException {
    if (sListener != null) {
